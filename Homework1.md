...
​		微处理器从1971年发展至2000年，其本身的形态与特点有了明显的变化。而遵从摩尔定律、从当前的处理器发展态势来看，微处理器仍有巨大的发展空间。本文中作者描述了微处理器的基本架构，回顾了1971至2000年的处理器发展、变化过程，并展望了微处理器的发展方向。

​		微处理器的设计在作者看来，相比于技术，其更是一种艺术，一种从各种利弊之间权衡的艺术。微处理器相关内容进行层次划分，分为了五个部分：目标问题、算法、程序、ISA与微处理器架构、电路设计、电子的运动。其中各个部分都影响着微处理器的设计。目标问题、算法、程序决定了微处理器运行的程序集的不同特征，进而需要处理器设计时进行不同方向的优化；ISA的选择直接影响这微处理器的实现方法，而对ISA的选择需要考虑成本、功耗等用户需求。而微处理器架构中，因为存储速度与处理器运行速度的差异，需要特别考虑指令与数据的供给方式。

​		微处理器的发展动力来自于很多方面，作者总结为了以下几个部分：新的用户需求、处理器的运行瓶颈以及运气成分。从1971至2000，处理器的变革主要有以下方面：
...
3. 加入分支预测。
4. 得益于晶体管密集程度增加，处理器加入片上浮点数运算单元以及其他专有功能的运算单元。
5. 加入乱序执行。
6. 随着处理器中运算单元增多，出现了丛集、分区的概念。
7. 片上可以加入多个处理器内核。
8. 提出同步多线程概念。
9. 允许处理器上不同单元以不同频率运行。

​		对于微处理器接下来的发展，作者坚信处理器中晶体管密度会大幅度提升、处理器运行频率也将飞跃。基于此，作者进行了一些展望：
...
​		另外，作者还提及需要更好用的CAD工具来辅助芯片设计。总结来说，作者认为微处理器的发展是没有止境的。
